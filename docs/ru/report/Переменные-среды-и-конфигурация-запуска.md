...
- `CODEXMANAGER_CODEX_IMAGE_GENERATION_AUTO_INJECT_TOOL`: автоматически добавляет tool `image_generation` в обычные `/v1/responses` запросы. Значение по умолчанию `1`, чтобы соответствовать поведению официального клиента Codex; явно переданный клиентом tool не дублируется.
...
3. Проверьте, не слишком ли велик `CODEXMANAGER_SSE_KEEPALIVE_INTERVAL_MS`, из-за чего промежуточный proxy считает SSE idle.
