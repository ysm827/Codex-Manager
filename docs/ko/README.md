...
- 현재 최신 릴리스: `v0.2.1` (2026-04-12 배포).
- 모델 관리 페이지가 공식 모델 카탈로그 편집 지점이 되었습니다. desktop 빌드에서는 변경 후 `~/.codex/models_cache.json`을 자동으로 덮어쓰고, web 배포에서는 `models_cache.json` 내보내기 버튼을 제공합니다.
- 모델 카탈로그는 이제 `display_name`, `description`, `supported_reasoning_levels`, `visibility`, `supported_in_api` 같은 핵심 메타데이터를 구조화해 저장합니다.
- Web 모드는 반드시 `codexmanager-web` 셸과 `/api/runtime`, `/api/rpc` 프록시를 통해 실행해야 하며, 정적 프런트엔드만 띄우는 방식은 더 이상 완전한 배포로 보지 않습니다.
- `refresh_token` 이 없는 free 계정은 usage 새로고침 시 refresh-token 400 오류를 더 이상 만들지 않습니다.
